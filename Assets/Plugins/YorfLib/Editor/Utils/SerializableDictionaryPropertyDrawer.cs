	...
	public class SerializableDictionaryPropertyDrawer : PropertyDrawer
	{
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			if (Event.current.type == EventType.Repaint)
			{
				...
				headerPos.y--;
				headerBackground.Draw(headerPos, false, false, false, false);
			}
			...
			if (property.isExpanded)
			{
				if (Event.current.type == EventType.Repaint)
				{
					float size = GetPropertyHeight(property, new GUIContent(""));
					GUIStyle background = "RL Background";
					Rect backgroundPos = position;
					backgroundPos.y += 15;
					backgroundPos.height = size - 16;
					background.Draw(backgroundPos, false, false, false, false);
				}
